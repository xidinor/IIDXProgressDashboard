...
## 2026-09-19〜20の決定・保留事項（本文に優先する追記）

- `original_data` の保存方針：旧DBの元行全体をJSONとして `play_history.raw_data` に保存し、その中の `original_data` は解析・整形せず元の文字列のまま保持する。これは旧PythonがPickleから取り出した行の `str(row)` 表現であり、Pickleバイナリではない。未解決行も `unresolved_imports.raw_data` に同様に保存する。通常の集計は変換済み列を使用し、元データは調査・再処理用とする。JSONの詳細構造と将来の再解析方法は実装時に定義する。
- 旧履歴は、選択した移行元の元行単位で全件保持する。同一プレイ日・曲名・難易度・プレイオプション・スコア・ミスカウントが一致する行も、短時間に記録された行も統合・間引きしない。同一内容の元行が3件あれば3件とも移行し、同じ入力の再取込ではその3件を増殖させない。二重登録防止の対象は同じ移行元の同じ元行のみとする。新形式への変換・検証は行い、不正・未解決行は元データと理由を保存して黙って破棄しない。複数DBの移行元選択は別途決める。
- 旧DBのplay_historyの日時はJST（UTC+09:00）。Legacy ImporterはJSTからUTCへ変換する。元日時の分精度の保持方法は引き続き検討する。Refluxの時刻解釈とは区別する。
- 2026-09-20確定：Phase 4はPRで提示されている暫定Reflux 1.17.0で取得したSession TSVを主対象とする。ユーザー報告では、IIDX本体のパッチ更新に伴うメモリ内オフセット不一致等により1.16.6は現在データを取得できない。1.16.6は旧版の形式比較用とし、現行環境での取得成功は完了条件にしない。1.17.0の実Sessionサンプルは配置済み。対象PR・commit・IIDXビルドと入力契約・互換性は[Phase 4 Issue #17](https://github.com/xidinor/IIDXProgressDashboard/issues/17)で確認する。正式リリース済み、Importer実装済み、取込検証済みとは扱わない。
- スコアレート・DJ LEVEL計算・表示に使うNotesの優先順位は「現行Notes > 当時Notes」。保存済みの実スコアと当時Notesは保持し、現行Notesで書き換えない。
- 当時Notesのスコアをオプション選択で表示する機能は希望事項としてPhase 6で検討する。譜面修正前後の履歴を集計・表示する範囲や、単なる分母切替にするかは未確定。Notes不明または0の場合の計算・表示・フォールバックも未定。
- この表示方針だけを根拠に、第10章等の照合時のnotes不一致検出を無効化しない。譜面修正に伴う差異を安全に識別する方法は別途設計する。

関連：[Phase 3 Issue #12](https://github.com/xidinor/IIDXProgressDashboard/issues/12)。本追記は方針の記録であり、実装完了を意味しない。

- 2026-09-20追加：ユーザー指定によりRefluxのSessionはファイル単位、日時は `date` 列（提供サンプルの第28列・最終列）とする。列はヘッダー名で読む。コピー・改名時のSession同一性、永続識別子、日時のUTC/Localは未確定。ユーザー確認済み：クリアランプは `lamp` 列、使用ゲージオプションは `gauge` 列。第16章の `lamp→clear_lamp` / `gauge→gauge_type` の対応を維持し、使用ゲージからクリア結果を推測しない。

- 2026-09-20実サンプル補完：追加Sessionで `lamp=AC / gauge=ASSIST EASY`、`lamp=EC / gauge=EASY`、DPの `style`・`style2` 両列の複数組合せを確認した。両オプション列を独立して保持する。ユーザー実機報告ではDP+BATTLE時は組合せによらずスコアが保存されない。追加ファイルの全行には正のスコアがあり、BATTLE・FLIP表記は確認できないため、行自体の出力抑止条件は上流実装で確認する。出力されないプレイの架空履歴生成や欠損スコアの0補完は行わない。
