...
実サンプルのuselocaltime設定は未確認であり、UTCを指定した検証は実際の絶対時刻が正しいとの確認ではない。1.16.6の現環境での取得成功や、ローカル統合バイナリの再ビルドは検証していない。DBロック長時間timeout・ディスク障害によるFAILEDログ保存失敗・プロセス強制終了の実機注入は未実施。
