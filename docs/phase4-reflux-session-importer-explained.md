...
2026-09-22追記：Issue #20第2節の[実マスター再検証・6行調査](phase4-followup-real-master-revalidation.md)を完了。正式Providerのマスターで68行登録・4行保留。別DBで旧66行登録・6行保留を再現後、マスター更新と同じSession IDでの再取込により2行だけ追加し、再々取込追加0を確認した。level不一致2行はRESOLVED、同じ正式曲名の複数tagに起因する4行はPENDINGを保持。原本・既存履歴・当時値・chart_idの保全を確認した。本番照合規則・DDLは変更せず、合成回帰3ケースを追加して全436テスト成功。通常DBの正式移行やフォローアップ全体の完了ではない。

実装時点では実サンプルのuselocaltime設定は未確認だった。2026-09-22にユーザーが出力当時もuselocaltime=falseだったことを確認したため、対象3ファイル・72行の時刻解釈はUTCと確定した。その後、別検証DBでの[元日時→UTC照合](phase4-followup-session-utc-validation.md)を完了し、今回の再検証でも一致を確認した。1.16.6の現環境での取得成功や、ローカル統合バイナリの再ビルドは検証していない。DBロック長時間timeout・ディスク障害によるFAILEDログ保存失敗・プロセス強制終了の実機注入は未実施。
