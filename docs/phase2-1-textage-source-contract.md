...
2026-09-21追記：ユーザー承認により、[全件調査で特定した17タグ](phase2-followup-local-textage-audit.md)を `firstemo` と同じ明示的例外とする。tag完全一致で曲・譜面候補から除外し、出現ファイルごとに `NON_PLAYABLE_EXCLUDED` を記録する。元入力はSourcesに保持する。全文構文検査は維持し、未承認tagの参照不足・不正値は引き続き全体失敗とする。カタログ外tag一般をスキップする規則ではない。
